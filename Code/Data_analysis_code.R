...
####okay no mediation just show no impact xxxx muow#####
#notes, requires removal of nas, results only hold with logged allocation.
#therefore log them earlier you mup.
pdf <- df %>% dplyr::select(deflated_per_person_benefits,
                            deflated_per_person_allocation,
                            deflated_per_person_allocation_la,
                            Private_Sector_Procurement_Spend,
...
z1 <- pgmm(log(treatable_mortality_rate) ~ lag(log(treatable_mortality_rate), 1:2) + lag(log(deflated_per_person_benefits), 0:1)
           | lag(log(treatable_mortality_rate), 2:99),
           data = pdf, effect = "twoways", model = "twosteps")

z2 <- pgmm(log(preventable_mortality_rate) ~ lag(log(preventable_mortality_rate), 1:2) + lag(log(deflated_per_person_benefits), 0:1)
           | lag(log(preventable_mortality_rate), 2:99),
           data = pdf, effect = "twoways", model = "twosteps")

z3 <- pgmm(log(avoidable_mortality_rate) ~ lag(log(avoidable_mortality_rate), 1:2) + lag(log(deflated_per_person_benefits), 0:1)
           | lag(log(avoidable_mortality_rate), 2:99),
           data = pdf, effect = "twoways", model = "twosteps")

z4 <- pgmm(log(treatable_mortality_rate) ~ lag(log(treatable_mortality_rate), 1:2) + lag(log(deflated_per_person_benefits), 0:1)
           + lag(Private_Sector_Procurement_Spend, 0:1) | lag(log(treatable_mortality_rate), 2:99),
           data = pdf, effect = "twoways", model = "twosteps")

z5 <- pgmm(log(preventable_mortality_rate) ~ lag(log(preventable_mortality_rate), 1:2) + lag(log(deflated_per_person_benefits), 0:1)
           + lag(Private_Sector_Procurement_Spend, 0:1) | lag(log(preventable_mortality_rate), 2:99),
           data = pdf, effect = "twoways", model = "twosteps")

z6 <- pgmm(log(avoidable_mortality_rate) ~ lag(log(avoidable_mortality_rate), 1:2) + lag(log(deflated_per_person_benefits), 0:1)
           + lag(Private_Sector_Procurement_Spend, 0:1) | lag(log(avoidable_mortality_rate), 2:99),
           data = pdf, effect = "twoways", model = "twosteps")

coef1 <- as.data.frame(summary(z1, robust=T)$coefficients)
coef1$upper_ci <- coef1$Estimate+(coef1$`Std. Error`*1.96)
coef1$lower_ci <- coef1$Estimate-(coef1$`Std. Error`*1.96)

coef2 <- as.data.frame(summary(z2, robust=T)$coefficients)
coef2$upper_ci <- coef2$Estimate+(coef2$`Std. Error`*1.96)
coef2$lower_ci <- coef2$Estimate-(coef2$`Std. Error`*1.96)

coef3 <- as.data.frame(summary(z3, robust=T)$coefficients)
coef3$upper_ci <- coef3$Estimate+(coef3$`Std. Error`*1.96)
coef3$lower_ci <- coef3$Estimate-(coef3$`Std. Error`*1.96)

coef4 <- as.data.frame(summary(z4, robust=T)$coefficients)
coef4$upper_ci <- coef4$Estimate+(coef4$`Std. Error`*1.96)
coef4$lower_ci <- coef4$Estimate-(coef4$`Std. Error`*1.96)

coef5 <- as.data.frame(summary(z5, robust=T)$coefficients)
coef5$upper_ci <- coef5$Estimate+(coef5$`Std. Error`*1.96)
coef5$lower_ci <- coef5$Estimate-(coef5$`Std. Error`*1.96)

coef6 <- as.data.frame(summary(z6, robust=T)$coefficients)
coef6$upper_ci <- coef6$Estimate+(coef6$`Std. Error`*1.96)
coef6$lower_ci <- coef6$Estimate-(coef6$`Std. Error`*1.96)


rownames(coef1) <- c("Lag Mortality [1]","Lag Mortality [2]", 
                     "Lag Benefits Allocation [0]", "Lag Benefits Allocation [1]" )

rownames(coef2) <- c("Lag Mortality [1]","Lag Mortality [2]", 
                     "Lag Benefits Allocation [0]", "Lag Benefits Allocation [1]" )


rownames(coef3) <- c("Lag Mortality [1]","Lag Mortality [2]", 
                     "Lag Benefits Allocation [0]", "Lag Benefits Allocation [1]" )

rownames(coef4) <- c("Lag Mortality [1]","Lag Mortality [2]", 
                     "Lag Benefits Allocation [0]", "Lag Benefits Allocation [1]",
                     "Lag For-Profit Outsourcing % [0]","Lag For-Profit Outsourcing % [1]")

rownames(coef5) <- c("Lag Mortality [1]","Lag Mortality [2]", 
                     "Lag Benefits Allocation [0]", "Lag Benefits Allocation [1]",
                     "Lag For-Profit Outsourcing % [0]","Lag For-Profit Outsourcing % [1]")

rownames(coef6) <- c("Lag Mortality [1]","Lag Mortality [2]", 
                     "Lag Benefits Allocation [0]", "Lag Benefits Allocation [1]",
                     "Lag For-Profit Outsourcing % [0]","Lag For-Profit Outsourcing % [1]")

coef1 <- coef1 %>% mutate_if(is.numeric, ~round(., 4))
coef2 <- coef2 %>% mutate_if(is.numeric, ~round(., 4))
coef3 <- coef3 %>% mutate_if(is.numeric, ~round(., 4))
coef4 <- coef4 %>% mutate_if(is.numeric, ~round(., 4))
coef5 <- coef5 %>% mutate_if(is.numeric, ~round(., 4))
coef6 <- coef6 %>% mutate_if(is.numeric, ~round(., 4))

coef1$Estimate <- paste0(coef1$Estimate, " [",coef1$lower_ci,"-",coef1$upper_ci ,"]"  ) 
coef2$Estimate <- paste0(coef2$Estimate, " [",coef2$lower_ci,"-",coef2$upper_ci ,"]"  ) 
coef3$Estimate <- paste0(coef3$Estimate, " [",coef3$lower_ci,"-",coef3$upper_ci ,"]"  ) 
coef4$Estimate <- paste0(coef4$Estimate, " [",coef4$lower_ci,"-",coef4$upper_ci ,"]"  ) 
coef5$Estimate <- paste0(coef5$Estimate, " [",coef5$lower_ci,"-",coef5$upper_ci ,"]"  ) 
coef6$Estimate <- paste0(coef6$Estimate, " [",coef6$lower_ci,"-",coef6$upper_ci ,"]"  ) 

library(str2str)
models <- cbind_fill(coef1, coef2, coef3, coef4, coef5,coef6)

models <- models %>% dplyr::select(-starts_with("Std"), -starts_with("z"),
                                   -starts_with("upper"), -starts_with("lower"))
models <- models[c(4,3,6,5,2,1),]

ns <- data.frame(length(summary(z1, robust=T)$fitted.values[summary(z1, robust=T)$fitted.values!=0]),
                 length(summary(z1, robust=T)$fitted.values[summary(z1, robust=T)$fitted.values!=0]),
                 length(summary(z2, robust=T)$fitted.values[summary(z2, robust=T)$fitted.values!=0]),
                 length(summary(z2, robust=T)$fitted.values[summary(z2, robust=T)$fitted.values!=0]),
                 length(summary(z3, robust=T)$fitted.values[summary(z3, robust=T)$fitted.values!=0]),
                 length(summary(z3, robust=T)$fitted.values[summary(z3, robust=T)$fitted.values!=0]),
                 length(summary(z4, robust=T)$fitted.values[summary(z4, robust=T)$fitted.values!=0]),
                 length(summary(z4, robust=T)$fitted.values[summary(z4, robust=T)$fitted.values!=0]),
                 length(summary(z5, robust=T)$fitted.values[summary(z5, robust=T)$fitted.values!=0]),
                 length(summary(z5, robust=T)$fitted.values[summary(z5, robust=T)$fitted.values!=0]),
                 length(summary(z6, robust=T)$fitted.values[summary(z6, robust=T)$fitted.values!=0]),
                 length(summary(z6, robust=T)$fitted.values[summary(z6, robust=T)$fitted.values!=0]))

cols.num <- names(ns[c(1:12)])
ns[,cols.num] <- sapply(ns[,cols.num], as.character)


names(models) <- c("Estimate [95% ci]", "P-value",
...
rownames(ns) <- "N"
names(ns) <- names(models)

models <- rbind(models, ns)

options(knitr.kable.NA = '')

kable(models,
      format = "html", valign = 't')  %>%
  add_header_above(c(" "=1,"Treatable Mortality" =2, "Preventable Moratlity" = 2,
                     "Avoidable Mortality" =2,"Treatable Mortality" =2, 
                     "Preventable Moratlity" = 2,"Avoidable Mortality" = 2))%>%
  kable_styling()%>%
  add_footnote(c("Table represents results from a 'Generalised Methods of Moment' dynamic panel model",
                 "CIs are calculated using robust SEs",
                 "Benefit allocation and mortality rates are log transformed") ,notation="alphabet")%>%
  row_spec(6, extra_css = "border-bottom: 1px solid") %>% 
  save_kable("Regressions/paper_3/gmm_updated.html")
...
three <-plm(log(Treatable_Mortality_Rate)~lag(Private_Sector_Procurement_Spend)*lag(deflated_per_person_allocation)+total_spend_10millions+GDHI_per_person+CCGpop, data=dfint, index=c("CCG_Name", "year") ,method="within", effect = "twoways")
four <-plm(log(Treatable_Mortality_Rate)~lag(Private_Sector_Procurement_Spend)*lag(deflated_per_person_allocation_la)+total_spend_10millions+GDHI_per_person+CCGpop, data=dfint, index=c("CCG_Name", "year") ,method="within", effect = "twoways")
five <-plm(log(Treatable_Mortality_Rate)~lag(Private_Sector_Procurement_Spend)*lag(deflated_per_person_benefits)+total_spend_10millions+GDHI_per_person+CCGpop, data=dfint, index=c("CCG_Name", "year") ,method="within", effect = "twoways")
...
cm <- c("lag(Private_Sector_Procurement_Spend)" = "For-profit outsourcing (%)",
        "lag(deflated_per_person_allocation)" = "CCG Allocation (? per capita)",
        "lag(deflated_per_person_allocation_la)" = "LA Allocation (? per capita)",
        "lag(deflated_per_person_benefits)" = "Benefit Allocation (? per capita)",
        "CCGpop" = "Population (n)",
        "total_spend_10millions" = "Total CCG spend (?10ms)",
        "GDHI_per_person" = "Avg. Household Income (?s)",
        "lag(Private_Sector_Procurement_Spend):lag(deflated_per_person_allocation)" = "Outsourcing*Allocation",
        "lag(Private_Sector_Procurement_Spend):lag(deflated_per_person_allocation_la)" = "Outsourcing*Allocation",
        "lag(Private_Sector_Procurement_Spend):lag(deflated_per_person_benefits)" = "Outsourcing*Allocation")
...
table <- modelsummary(list("ln. Treatable Mortality [.95 ci]"=onesum,"p-value"=onesum,"ln. Treatable Mortality [.95 ci]"=twosum,"p-value"=twosum,"ln. Treatable Mortality [.95 ci]"=threesum,"p-value"=threesum,"ln. Treatable Mortality [.95 ci]"=foursum,"p-value"=foursum,"ln. Treatable Mortality [.95 ci]"=fivesum,"p-value"=fivesum),
  ...
  gtsave(table, "Regressions/paper_3/FE_interactions.html")
...
####latest, lm works, plm doesnt - think it's not correctly lagging in lm....ughhh manual lag baby####
df$logged_prevent <- log(df$Preventatable_Mortality_Rate)
df$logged_treat <- log(df$Treatable_Mortality_Rate)

pdf <- df %>% dplyr::select(lag_check_private,
                            lagged_bens,
                            lagged_prevent,
                            logged_treat,
                            Lagged_Treatable_Mortality_Rate,
                            CCG_Name,
                            year,
                            logged_prevent)%>%
  dplyr::filter(!is.na(lag_check_private),
                !is.na(lagged_bens),
                !is.na(CCG_Name),
                !is.na(year),
                !is.na(lagged_prevent),
                !is.na(logged_prevent),
                !is.na(logged_treat),
                !is.na(Lagged_Treatable_Mortality_Rate),
                lag_check_private>=0,
                year!="2013")

m_bens <- lm(
  lag_check_private~ lagged_bens + lagged_prevent
  + Lagged_GDHI_per_person+CCGpop+CCG_Name+year,
  data = df ) 

summary(plm(log(Treatable_Mortality_Rate) ~ lag(Treatable_Mortality_Rate, 1:2) + 
              lag(log(deflated_per_person_allocation), 0:1)+
              lag(Private_Sector_Procurement_Spend, 0:1) , effect = "twoway", model = "within",
            data = pdf))

m_c_bens <- lm(
  logged_treat ~  lag_check_private+ 
    CCG_Name+year+Lagged_Treatable_Mortality_Rate+
    log(lagged_bens),
  data = pdf  ) 
...
med.bens <- 
  mediation::mediate(m_bens, m_c_bens, 
                     treat = 'lagged_bens', 
                     mediator = 'lag_check_private', 
...
m_ccg <- lm(
  lag_check_private~ lagged_ccg + Lagged_Treatable_Mortality_Rate
  + Lagged_GDHI_per_person+CCGpop+CCG_Name+year,
  data = df ) 

m_c_ccg <- lm(
  logged_treat ~ lagged_ccg + lag_check_private+ 
    Lagged_GDHI_per_person+ CCG_Name+year+Lagged_Treatable_Mortality_Rate+CCG_Name + year ,
  data = df  ) 

df.residual(m_c_j)

med.ccg <- 
  mediation::mediate(m_ccg, m_c_ccg, 
                     treat = 'lagged_ccg', 
                     mediator = 'lag_check_private', 
                     robustSE = F, boot=F, sims=100, dropobs=T) 
...
z1 <- pgmm(log(treatable_mortality_rate) ~ lag(log(treatable_mortality_rate), 1:2) + lag(log(deflated_per_person_allocation_la), 0:1)
           | lag(log(treatable_mortality_rate), 2:99),
           data = pdf, effect = "twoways", model = "twosteps")

z2 <- pgmm(log(preventable_mortality_rate) ~ lag(log(preventable_mortality_rate), 1:2) + lag(log(deflated_per_person_allocation_la), 0:1)
           | lag(log(preventable_mortality_rate), 2:99),
           data = pdf, effect = "twoways", model = "twosteps")

z3 <- pgmm(log(avoidable_mortality_rate) ~ lag(log(avoidable_mortality_rate), 1:2) + lag(log(deflated_per_person_allocation_la), 0:1)
           | lag(log(avoidable_mortality_rate), 2:99),
           data = pdf, effect = "twoways", model = "twosteps")

z4 <- pgmm(log(treatable_mortality_rate) ~ lag(log(treatable_mortality_rate), 1:2) + lag(log(deflated_per_person_allocation_la), 0:1)
           + lag(Private_Sector_Procurement_Spend, 0:1) | lag(log(treatable_mortality_rate), 2:99),
           data = pdf, effect = "twoways", model = "twosteps")

z5 <- pgmm(log(preventable_mortality_rate) ~ lag(log(preventable_mortality_rate), 1:2) + lag(log(deflated_per_person_allocation_la), 0:1)
           + lag(Private_Sector_Procurement_Spend, 0:1) | lag(log(preventable_mortality_rate), 2:99),
           data = pdf, effect = "twoways", model = "twosteps")

z6 <- pgmm(log(avoidable_mortality_rate) ~ lag(log(avoidable_mortality_rate), 1:2) + lag(log(deflated_per_person_allocation_la), 0:1)
           + lag(Private_Sector_Procurement_Spend, 0:1) | lag(log(avoidable_mortality_rate), 2:99),
           data = pdf, effect = "twoways", model = "twosteps")
...
rownames(coef1) <- c("Lag Mortality [1]","Lag Mortality [2]", 
                     "Lag LA Allocation [0]", "Lag LA Allocation [1]" )
...
kable(models,
  ...
  add_footnote(c("Table represents results from a 'Generalised Methods of Moment' dynamic panel model",
                 ...
                 "Benefit allocation and mortality rates are log transformed") ,notation="alphabet")%>%
  row_spec(6, extra_css = "border-bottom: 1px solid") %>% 
  save_kable("Regressions/paper_3/gmm_la_allocation_updated.html")
...
z1 <- pgmm(log(treatable_mortality_rate) ~ lag(log(treatable_mortality_rate), 1:2) + lag(log(deflated_per_person_allocation), 0:1)
           | lag(log(treatable_mortality_rate), 2:99),
           data = pdf, effect = "twoways", model = "twosteps")

z2 <- pgmm(log(preventable_mortality_rate) ~ lag(log(preventable_mortality_rate), 1:2) + lag(log(deflated_per_person_allocation), 0:1)
           | lag(log(preventable_mortality_rate), 2:99),
           data = pdf, effect = "twoways", model = "twosteps")

z3 <- pgmm(log(avoidable_mortality_rate) ~ lag(log(avoidable_mortality_rate), 1:2) + lag(log(deflated_per_person_allocation), 0:1)
           | lag(log(avoidable_mortality_rate), 2:99),
           data = pdf, effect = "twoways", model = "twosteps")

z4 <- pgmm(log(treatable_mortality_rate) ~ lag(log(treatable_mortality_rate), 1:2) + lag(log(deflated_per_person_allocation), 0:1)
           + lag(Private_Sector_Procurement_Spend, 0:1) | lag(log(treatable_mortality_rate), 2:99),
           data = pdf, effect = "twoways", model = "twosteps")

z5 <- pgmm(log(preventable_mortality_rate) ~ lag(log(preventable_mortality_rate), 1:2) + lag(log(deflated_per_person_allocation), 0:1)
           + lag(Private_Sector_Procurement_Spend, 0:1) | lag(log(preventable_mortality_rate), 2:99),
           data = pdf, effect = "twoways", model = "twosteps")

z6 <- pgmm(log(avoidable_mortality_rate) ~ lag(log(avoidable_mortality_rate), 1:2) + lag(log(deflated_per_person_allocation), 0:1)
           + lag(Private_Sector_Procurement_Spend, 0:1) | lag(log(avoidable_mortality_rate), 2:99),
           data = pdf, effect = "twoways", model = "twosteps")
...
rownames(coef1) <- c("Lag Mortality [1]","Lag Mortality [2]", 
                     "Lag CCG Allocation [0]", "Lag CCG Allocation [1]" )
...
kable(models,
  ...
  add_footnote(c("Table represents results from a 'Generalised Methods of Moment' dynamic panel model",
                 ...
                 "Benefit allocation and mortality rates are log transformed") ,notation="alphabet")%>%
  row_spec(6, extra_css = "border-bottom: 1px solid") %>% 
  save_kable("Regressions/paper_3/gmm_ccg_allocation_updated.html")
